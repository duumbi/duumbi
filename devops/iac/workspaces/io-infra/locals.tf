locals {
  ...
  aks_name         = lower(format("%s-%s-%s-%s-aks", var.organization, var.project, var.environment, var.location_base.short_name))
  aks_node_rg_name = lower(format("%s-%s-%s-%s-aks-node-rg", var.organization, var.project, var.environment, var.location_base.short_name))
