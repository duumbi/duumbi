locals {
  ...
  aks_rg_name      = lower(format("%s-%s-%s-%s-aks-rg", var.organization, var.project, var.environment, local.location_short_name))
  aks_vnet_name    = lower(format("%s-%s-%s-%s-aks-vnet", var.organization, var.project, var.environment, local.location_short_name))
